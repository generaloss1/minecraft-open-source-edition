...
public class MapGenOLDCaves extends MapGenOLD{
    ...
    protected void func_870_a(int i, int j, ClientBlock[] blocks, double d, double d1, double d2){
        releaseEntitySkin(i, j, blocks, d, d1, d2, 1.0F + rand.nextFloat() * 6F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void releaseEntitySkin(int i, int j, ClientBlock[] blocks, double d, double d1, double d2, float f, float f1, float f2, int k, int l, double d3){
        double d4 = i * 16 + 8;
        double d5 = j * 16 + 8;
        float f3 = 0.0F;
        float f4 = 0.0F;
        ...
        mainCycle:
        for(; k < l; k++){
            double d6 = 1.5D + (Math.sin(((float) k * 3.141593F) / (float) l) * f * 1.0F);
            double d7 = d6 * d3;
            float f5 = Mathc.cos(f2);
            float f6 = Mathc.sin(f2);
            d += Mathc.cos(f1) * f5;
            d1 += f6;
            d2 += Mathc.sin(f1) * f5;
            if(flag1){
            ...
            int d8 = Mathc.floor(d - d6) - i * 16 - 1;
            int k1 = (Mathc.floor(d + d6) - i * 16) + 1;
            int d9 = Mathc.floor(d1 - d7) - 1;
            int l1 = Mathc.floor(d1 + d7) + 1;
            int d10 = Mathc.floor(d2 - d6) - j * 16 - 1;
            int i2 = (Mathc.floor(d2 + d6) - j * 16) + 1;
            if(d8 < 0){
                d8 = 0;
            }
            if(k1 > 16){
            ...
            boolean flag2 = false;
            for(int j2 = d8; j2 < k1; j2++){
                for(int l2 = d10; l2 < i2; l2++){
                    for(int i3 = l1 + 1; i3 >= d9 - 1; i3--){
                        int j3 = (j2 * 16 + l2) * 128 + i3;
                        if(i3 >= 128) // if(i3 < 0 || i3 >= 128)
                            continue;

                        if(blocks[j3] == ClientBlocks.WATER){
                            continue mainCycle;
                        }
                ...
                for(int k3 = d10; k3 < i2; k3++){
                    ...
                    do{
                        ...
                        if(d14 > -0.69999999999999996D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D){
                            ClientBlock byte0 = blocks[l3];

                            if(byte0 == ClientBlocks.GRASS_BLOCK){
                                flag3 = true;
                            }
                            if(byte0 == ClientBlocks.STONE || byte0 == ClientBlocks.DIRT || byte0 == ClientBlocks.GRASS_BLOCK){
                                if(i4 < 10){
                                    // blocks[l3] = ClientBlocks.flowing_lava;
                                }else{
                                    blocks[l3] = ClientBlocks.AIR;
                                    if(flag3 && blocks[l3 - 1] == ClientBlocks.DIRT){
                                        blocks[l3 - 1] = ClientBlocks.GRASS_BLOCK;
                                    }
                                }
                            }
                        }
