...
/**
 * This is a Voronoi noise generator, originally from <a href="https://github.com/TJHJava/libnoiseforjava">libnoiseforjava github</a>
...
public class CellNoise{
    ...
    public CellNoise(long seed, int distanceMethod){
        this.seed = seed;
    ...
    private double getDistance(double xDist, double yDist, double zDist){
        return switch(distanceMethod){
            case 0 -> Math.sqrt(xDist * xDist + yDist * yDist + zDist * zDist) / SQRT_3; //Approximation (for speed) of elucidean (regular) distance
    ...
    public float noise(double x, double z, double frequency){
        ...
        if(useDistance){
            ...
            return ((float) CellNoise.valueNoise2D((int) (Math.floor(xCandidate)), (int) (Math.floor(zCandidate)), seed));
    }
    ...
    public double noise(double x, double y, double z, double frequency){
        ...
        if(useDistance){
            ...
            return (CellNoise.valueNoise3D((int) (Math.floor(xCandidate)), (int) (Math.floor(yCandidate)), (int) (Math.floor(zCandidate)), seed));
